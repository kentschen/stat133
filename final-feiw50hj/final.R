...
name <- "Kent Chen"
github.name <- "kentschen"
email.address <- "kentchen@berkeley.edu"
...
# i.e. each element is one of the numbers 1 through 6, with equal probability.
  set.seed(31415)
#v <- <your code here>
v <- replicate(100, sum(sample(1:6), size = 1/6, replace = T))
...
#cw2 <- <your code here>
cw2 <- ChickWeight[ ChickWeight$Time == "0" & ChickWeight$weight > 42, ]

# [1 pt]
# Create [cw3] a single number, the average chick weight at time 0
#cw3 <- <your code here>
cw3 <- mean(ChickWeight$Time == "0")  
...
# 
# t2 <- <your code here>
t2 <- table(infants$smoke, infants$number, infants$ed == "College")
...
bdiff <- (infants$bwt > 35)-(infants$bwt < 35)
...
plot(mtcars$disp ~ mtcars$wt, xlab = "Engine Displacement in cu. in.", ylab = "Weight of car in lb/100", col=as.numeric(mtcars$am))
...
plot(WorldBank$life.expectancy ~ WorldBank$fertility.rate, pch = ".", col = "red", xlab = "Fertility Rate", ylab = "Life Expectancy")
abline(v=2.1)
...
max.diff.rain <- abs(diff((i-(i+1)), length(1:5)))
